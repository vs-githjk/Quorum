...
function App() {
  ...
  // Ref so the ws onmessage closure always sees the latest currentCard
  const currentCardRef = useRef<QuorumCard | null>(null);
  ...
  useEffect(() => {
    ...
    function connect() {
      ...
      ws.onmessage = (event) => {
        if (cancelled) return;
        try {
          const card: QuorumCard = JSON.parse(event.data);

          // Move whatever is currently shown to the sidebar
          const prev = currentCardRef.current;
          if (prev) {
            setSidebarCards((cards) => {
              if (cards.find((c) => c.id === prev.id)) return cards;
              return [...cards, prev];
            });
          }
    ...
    <div className="flex h-screen bg-[#0a0a0a]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="px-8 py-5 border-b border-white/[0.04] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div
              className={`w-2 h-2 rounded-full ${
                connected ? "bg-emerald-500 animate-pulse" : "bg-white/20"
              }`}
            />
            <span
          ...
          <span
            className={`text-[11px] font-light px-4 py-1.5 rounded-full tracking-wide ${
              connected
